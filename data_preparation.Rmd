...
for(category in char_to_factors[-1]) {
    category_arr <- billing_df[[category]]
    num_levels <- length(levels(category_arr))
    auxillaries <- cbind(
      auxillaries,
      to_categorical(
        na_to_zero(as.numeric(category_arr) - 1),
        num_levels
      )
    )
}
