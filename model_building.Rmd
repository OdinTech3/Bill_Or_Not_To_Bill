...
```{r}
set.seed(2019)
train_index <- create_data_partition(billing_notes_data)

x_train <- billing_notes_data[train_index,]
x_val <- billing_notes_data[-train_index,]
...
```{r}
set.seed(2019)
train_index <- create_data_partition(item_desc_data)

x_train <- item_desc_data[train_index,]
x_val <- item_desc_data[-train_index,]
...
ct_input_layer <- layer_input(
  shape = c(CONSTANTS$NUM_SAMPLES, CONSTANTS$MAX_LEN),
  dtype = "int32",
  name = "Call Text"
)
