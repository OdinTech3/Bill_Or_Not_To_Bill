...
### 1.1 Splitting `Invoiced` into Training and Validation

```{r}
invoiced_train <- invoiced[train_index,] %>% as.array()
invoiced_val <- invoiced[-train_index,] %>% as.array()

cat(" Shape of Call Text Train:", dim(invoiced_train), "\n",
    "Shape of Call Text Val:", dim(invoiced_val))
```

### 1.2 Splitting `Call Text` into Training and Validation
...
model_result <- model %>%
  evaluate(
    list(call_text_val, billing_notes_val, auxillaries_val),
    invoiced_val
  )
```

```{r}
cat('Val loss:', model_result$loss, "\n")
cat('Val accuracy:', model_result$acc, "\n")
```
...
```{r}
roc_result <- roc(invoiced_val, as.vector(pred_probs))
...
```{r}
pred_class <- as.numeric(pred_probs > .50) %>% as.factor()

confusionMatrix(
  pred_class,
  as.factor(invoiced_val),
  mode = "prec_recall",
  dnn = c("Not Bill", "Bill")
)
```
